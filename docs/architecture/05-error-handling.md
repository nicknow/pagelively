...
class AppError extends Error {
  ...
  status: number; // 400 | 403 | 404 | 405 | 409 | 413 | 422 | 500
...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| 400    | `invalid_id`, `invalid_slug`, `invalid_manifest`, `invalid_json`, `invalid_content`, `invalid_format`, `ambiguous_entry`, `path_traversal`, `unsupported_type`, `missing_field`, `malformed_json`, `invalid_home_mode`, `invalid_password`, `invalid_form_data` | Malformed input that no retry-with-fixes can pass without changes. Includes **ambiguous entry detection** (OQ-11: server 400s with a clear message; the UI forces an entry picker instead) and the JSON paste validation surface (`invalid_json`, `invalid_content`, `invalid_format`, OQ-17/T4). **S23 (ADR 0041):** `invalid_password` — missing/blank/out-of-bounds password on the unlock surface or the admin password field; `invalid_form_data` — non-form or malformed unlock POST body (also the pre-existing S17 multipart parse guard). | §10, §17, S17/S23 AC |
...
| 405    | `method_not_allowed`                                                                                                                                                                                                                                            | Route hit with a method it does not support. Public: the unlock surface answers 405 for every method other than GET/POST (ADR 0041 d3). Admin: `/api/pages` with an unmatched method (src/index.ts). No `Allow` header is emitted.                                                                                                                                                                                                                                                                                                                 | S23 AC               |
...
>
> **`invalid_rev` is deliberately dual-use (S23-C, ADR 0041 d8).** The 500-class use above is
> `buildR2Key`/`nextRev`'s internal-invariant guard (`rev.ts`). Separately, `serveAsset`'s
> `parseRev` throws the **same code** as a **400** for client-supplied rev input on the asset
> URL (`/assets/pages/{id}/{rev}/…`). One code, two statuses — the status at the boundary is
> what distinguishes client-supplied 400 from internal-invariant 500. The reuse is
> intentional so it never reads as a typo (see also architecture 02's `buildR2Key` note).
